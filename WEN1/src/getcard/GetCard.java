...
//		******** cardNum은 오름차순으로 정렬되므로 원래의 랜덤한 순서의 카드 배열 retVal ***********
		...
		// 오름차순으로 정렬 된 Com 카드 ( 0 ~ 7 )
		compCard = ArrangeCard(0, 7, cardNum);
		// 오름차순으로 정렬 된 user 카드 ( 7 ~ 14 )
...
//		************* 결과판단 *************
	...
	public int[] ArrangeCard(int startIndex, int endIndex, int[] cardNum) {
		int[] tempCard = new int[7];	// 재배치된 카드를 임시로 저장할 배열
		int reIndex = 0;				// 임의 인덱스
		for (int i = startIndex; i < endIndex; i++) {
			...
			}		//cardNum 배열에서 받았던 카드들은 각각의 고유 번호를 갖고 이를 비교해
					// 낮은 숫자의 카드가 앞으로 오게 정렬하는 for문인것 같습니다.
			tempCard[reIndex] = cardNum[i];
			reIndex++;
		}
		return tempCard;		// 다 정렬된 tempCard 배열을 반환
	}
